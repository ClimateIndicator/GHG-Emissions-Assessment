...
data_gcb_co2_carb <- data_gcb_co2_carb %>%
  ...
  select(year=Year,value)
data_gcb_co2_carb <- data_gcb_co2_carb %>% 
  mutate(source="GCB v2024 [cement carbonation]") %>% 
  mutate(value=value*3.664) %>% 
  mutate(value=-value) %>% 
  mutate(var="co2_ffi") %>% 
  mutate(units="GtCO2") %>% 
...
names(data_gcb_co2_luc) <- c("year","GCB v2024*","BLUE","H&C","OSCAR","LUCE")
...
data_edgar_co2_ffi <- gather(data_edgar_co2_ffi,year,value,Y_1970:Y_2023)
...
data_edgar_ch4 <- gather(data_edgar_ch4,year,value,Y_1970:Y_2023)
...
data_edgar_n2o <- gather(data_edgar_n2o,year,value,Y_1970:Y_2023)
...
data_edgar_fgases <- gather(data_edgar_fgases,year,value,Y_1990:Y_2023)
...
#data_ceds_ch4 <- read.csv("sources/CEDS/CH4_CEDS_emissions_by_country_v2024_07_08.csv")
...
#data_ceds_co2_ffi <- read.csv("sources/CEDS/CO2_CEDS_emissions_by_country_v2024_07_08.csv")
...
#data_ceds_n2o <- read.csv("sources/CEDS/N2O_CEDS_emissions_by_country_v2024_07_08.csv")
...
#data_primap <- read.csv("sources/PRIMAP/Guetschow_et_al_2024a-PRIMAP-hist_v2.6_final_13-Sep-2024.csv")
...
data_primap_cr_n2o <- data_primap %>% 
  ...
  summarise(value=sum(value,na.rm = TRUE)/1000) %>% 
  mutate(units="MtN2O") %>% 
  #mutate(source="PRIMAP Hist-CR v2.6.1") %>% 
  mutate(source="PRIMAP CR v2.6.1") %>% 
  mutate(var="n2o")
...
data_primap_tp_n2o <- data_primap %>% 
  ...
  summarise(value=sum(value,na.rm = TRUE)/1000) %>% 
  mutate(units="MtN2O") %>% 
  #mutate(source="PRIMAP Hist-TP v2.6.1*") %>% 
  mutate(source="PRIMAP TP v2.6.1*") %>% 
  mutate(var="n2o")
...
## Still using old dataset due to GFED5 being incomplete as of 20.02.2024
...
data_gfed_ch4 <- data_gfed_ch4 %>%
  ...
  mutate(value=value/1e6) %>%
  mutate(source="GFED v4.1 [biomass fires]") %>%
  mutate(units="MtCH4") %>%
...
data_sectors <- data
save(data_sectors,file="results/data/data_sectors_2025.RData")
...
p_ghg_1 <- data_ghg %>% 
  ...
  theme_wl() +
  scale_x_continuous(limits=c(1970,2023),breaks=c(1970,1980,1990,2000,2010,2023),expand = c(0, 0)) +
...
data_ghg_labels <- data_ghg %>% filter(year==2023) %>% select(year=year,label,value) %>% arrange(desc(label))
...
p_ghg_2 <- 
  ...
  geom_text_repel(
    ...
    color="#636363") +
  theme_wl_empty() +
  scale_x_continuous(limits=c(2023,2045),breaks=c(2023,2045),expand = c(0, 0)) +
  scale_y_continuous(limits=c(0,60),breaks=c(0,10,20,30,40,50,60),expand = c(0, 0)) +
  expand_limits(x = 0, y = 0) +
...
p_co2_luc_1 <- data %>% 
  ...
  geom_path(data=data %>% filter(var=="co2_luc") %>% filter(source=="GCB v2024*"),size=1) +
  geom_path(data=data %>% filter(var=="co2_luc") %>% filter(source!="GCB v2024*"),size=1,linetype=7,alpha=0.55) +
  coord_cartesian(clip = 'off') +
  theme_wl() +
  scale_x_continuous(limits=c(1970,2023),breaks=c(1970,1980,1990,2000,2010,2023),expand = c(0, 0)) +
  scale_y_continuous(limits=c(-4,11),breaks=c(-2,0,2,4,6,8,10),expand = c(0, 0)) +
  scale_color_manual(values=colors,drop=FALSE) + 
  expand_limits(x = 0, y = 0) +
  theme(axis.title = element_blank(),
        panel.border = element_blank(),
...
```{r plot_emissions_2025,fig.height=9,fig.width=10}
...
ggsave(plot,filename = "plot_emissions_2025.png",path = "results/",device = "png",height = 9,width=10,dpi=300)
ggsave(plot,filename = "plot_emissions_2025.pdf",path = "results/",device = cairo_pdf,height = 9,width=10)
ggsave(plot,filename = "plot_emissions_2025.svg",path = "results/",device = 'svg',height = 9,width=10)
...
data_table <- data_ghg %>% 
  ...
  mutate(cut=ifelse(year>=2014 & year <=2023,"2014-2023",cut))
...
data_table <- data_table %>% 
  ungroup() %>% 
  add_row(label="CO2-FFI",cut="2024 (projection)",value="38.1±3") %>% 
  add_row(label="CO2-LULUCF",cut="2024 (projection)",value="4.4±3.1")
...
write.xlsx(data_table,'results/section_2_2_table_2025.xlsx')
write.csv(data_table,'results/section_2_2_table_2025.csv',row.names = FALSE)
...
saveWorkbook(wb,'results/ghg_emissions_co2e_2025.xlsx',overwrite=T)

data_2025 <- data
data_ghg_2025 <- data_ghg
save(data_2025,data_ghg_2025,file="results/data/data_2025.RData")
...
plot <- data_conventions %>% ggplot(.,aes(x=value,y=convention,fill=component)) +
  ...
  labs(subtitle=bquote("Gt" ~CO[2]* "e in 2023"))
plot


## Save figure
#  
ggsave(plot,filename = "plot_emissions_differences_2025.png",path = "results/",device = "png",height = 2.5,width=9,dpi=300)
ggsave(plot,filename = "plot_emissions_differences_2025.pdf",path = "results/",device = cairo_pdf,height = 2.5,width=9)
ggsave(plot,filename = "plot_emissions_differences_2025.svg",path = "results/",device = 'svg',height = 2.5,width=9)
