...
data_gcb_co2_luc <- read.xlsx("sources/GCB/Global_Carbon_Budget_2024_v1.0-1.xlsx",sheet=5,startRow = 39,cols = c(1,2,9,14,19))

names(data_gcb_co2_luc) <- c("year","GCB v2024*","BLUE","H&C","OSCAR")
...
p_co2_2 <- data_labels %>%
  filter(var=="co2_ffi") %>%
  ggplot(.,aes(x=year,y=value,color=source,group=source,label=str_wrap(source,25))) +
  geom_text_repel(
    nudge_x      = 3.5,
    direction    = "y",
    hjust        = 0,
...
p_n2o_2 <- data_labels %>% 
  filter(var=="n2o") %>% 
  ggplot(.,aes(x=year,y=value,color=source,group=source,label=source,nudge_y=ifelse(source!="GFED v4.1",5,0))) +
  geom_text_repel(
    nudge_x      = 3.5,
    direction    = "y",
    hjust        = 0,
...
p_n2o <- p_fgases_1 + p_fgases_2 + plot_layout(widths=c(4,2.1))
p_n2o
...
save(data_2024,data_ghg_2024,file="results/data/data_2025.RData")
